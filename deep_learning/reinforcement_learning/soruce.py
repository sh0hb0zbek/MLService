...
def define_network():
    ...
    while True:
        ...
        layer_type = input("Name of module: ")
        if layer_type in layer_orders.keys(): layer_orders[layer_type] += 1
        else: layer_orders[layer_type] = 1
        layers.appand([layer_type, f"{layer_type}{layer_orders[layer_type]}", get_args(inspect.signature(eval(f"nn.{layer_type}")))])
    
    for layer in layers:
        network.add_module(layer[1], eval(f"nn.{layer[0]}")(**layer[2]))
    return network
...
def test_atari_main(env_name, seed, buffer_size=10**4,
                    timesteps_per_epoch=1, batch_size=32, total_steps=50_000, lr=1e-4,
                   start_epsilon=1, end_epsilon=0.05, eps_decay_final_step=20_000,
                   loss_freq=20, refresh_target_network_freq=100, eval_freq=None, max_grad_norm=5000):
    if seed is not None:
...
def test_ddqn_main(env_name, seed=None, buffer_size=10**4,
                      timesteps_per_epoch=1, batch_size=32, total_steps=50_000, lr=1e-4,
                      start_epsilon=1, end_epsilon=0.05, eps_decay_final_step=20_000,
                      loss_freq=20, refresh_target_network_freq=100, eval_freq=None, max_grad_norm=5000):
    if seed is not None:
...
def test_ppo_main(env_name, total_timesteps, do_evaluate=False):
    # environment setup
    env = make_env(env_name)
...
def test_reinforce_main(env_name, total_steps, eval_freq=100, lr=1e-3):
    # environment setup
    env = make_env(env_name)
...
def test_ddpg_main(env_name, seed=None, buffer_size=10**4,
                      timesteps_per_epoch=1, batch_size=32, total_steps=50_000, lr=1e-4,
                      start_epsilon=1, end_epsilon=0.05, eps_decay_final_step=20_000,
                      loss_freq=20, refresh_target_network_freq=100, eval_freq=None, max_grad_norm=5000):
    if seed is not None:

...
class Model():
    def __init__(self, model_name, **args):
        ...
        self.model_check(self.model_name)
        self.model_args = dict()
        self.set_args(args)
        print(self.model_args)
    
    def model_check(self, model_name):
        models = {
            'regressor': {
                'linear_model': [
                    'ARDRegression',
        ...
        for model_type in models.keys():
            for model_family in models[model_type].keys():
                if model_name in models[model_type][model_family]:
                    self.model_type = model_type
                    self.model_f = eval('sklearn.'+model_family+'.'+model_name)
                    return
    ...
    def plot_reg(self, X, target):
        pred = self.predict(X)
        print(f'Mean Squared Error: {sklearn.metrics.mean_squared_error(target, pred)}')
